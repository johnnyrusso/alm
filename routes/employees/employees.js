...
const express = require("express"),
  ...
  _ = require("lodash"),
  { exists, isTrue } = require("../../config/functions"),
  dateTime = require("../../config/dateFuncs"),
  { pool, promQuery, selectAll } = require("../../config/db");
...
router.get("/login", forwardAuthenticated, (req, res) => {
  const cssFiles = ["styles", "login"];
  res.render("./employees/login", {
    title: "ALM - Staff Login",
    cssFiles: cssFiles
  });
});
...
router.get(["/", "/dashboard"], ensureAuthenticated, (req, res) => {
  const dashboardInfoSql = `SELECT week_ending\
    FROM timesheets\
    WHERE employee_id = ?\
  ...
  pool
    .promise()
    .query(dashboardInfoSql, [req.user.id])
    .then(([rows, fields]) => {
      return rows;
    })
    .catch(console.log)
...
router.get("/timesheets/new", ensureAuthenticated, (req, res) => {
  const cssFiles = [
    "multi-step-form",
    "styles",
    "https://res.cloudinary.com/dxfq3iotg/raw/upload/v1581152092/smartwizard/smart_wizard.min",
    "https://res.cloudinary.com/dxfq3iotg/raw/upload/v1581152092/smartwizard/smart_wizard_theme_dots.min"
  ];
  const jsFiles = [
    "https://cdn.jsdelivr.net/npm/bootstrap-vue@2.4.2/dist/bootstrap-vue.common.min",
    "https://res.cloudinary.com/dxfq3iotg/raw/upload/v1581152197/smartwizard/jquery.smartWizard.min.js"
  ];

  const currWeekEnding = moment(dateTime.nextWeekday(moment(), 7)).format(
    "YYYY-MM-DD"
  );
  ...
  const sql = `SELECT * FROM timesheets
                ...
                ORDER BY week_ending DESC
                LIMIT 1`;
  console.log(currWeekEnding, req.user.id);

  pool
    .promise()
    .query(sql, [currWeekEnding, req.user.id])
    .then(([rows, fields]) => {
      {
        return rows;
      }
    ...
    .then(rows => {
      if (rows.length === 0) {
        res.render("./employees/timesheets/new2", {
...
router.post("/timesheets", ensureAuthenticated, (req, res) => {
  ...
  const weekEnding = new Date(
    dateTime.nextWeekday(moment(timesheet.dateWorked), 7)
  );
  ...
  pool.query(
    ...
    (err, timesheetRow) => {
      ...
      if (!timesheetRow.length) {
        ...
        pool.query(
          updateTimesheetSql,
          [totalMinutes, req.user.id, weekEnding],
          (err, updatedTimesheetRow) => {
            if (err) throw err;
            // Run SELECT Query For Locations
            pool.query(
              selectLocationSql,
              [req.user.id, timesheet.location, isoDateWorked],
              (err, row) => {
                if (err) throw err;
                // Create tasksArray And Loop Through HTML Form Data, Pushing Data To tasksArr
                let tasksArr = [];
...
router.get("/receipts/new", (req, res) => {
  res.render("./employees/timesheets/newReceipt", {
    title: "ALM: New Receipt",
    cssFiles: "",
    firstName: req.user.first_name,
    lastName: req.user.last_name
  });
...
router.get("/timesheets/view/current", (req, res) => {
  ...
  pool
    .promise()
    .query(timesheetJoinSql, [req.user.id, req.query.week_ending])
    .then(([rows, fields]) => {
      return rows;
    })
    .catch(console.log)
    .then(rows => {
      ...
      let locationsWorked = [
        ...new Set(rows.map(locations => locations.location))
      ];
      console.log(rows);
      return res.send({
        rows,
  ...
  (req, res) => {
    pool.query(
      ...
      function(err, row) {
        ...
        pool.query(
          "SELECT DISTINCT `week_ending` FROM `locations` WHERE `employee_id`= ? ORDER BY `week_ending` DESC",
          [req.params.staff_id],
...
// Admin View For Employee Timesheet
router.get(
  "/admin/timesheets/view/:staff_id/:week_ending",
  ensureAuthenticated,
  (req, res) => {
    let timesheetData = [req.params.staff_id, req.params.week_ending];
    // Select One Timesheet
    const timesheetJoinSql = `
         SELECT TS.total_minutes_worked, TS.week_ending, L.location, L.date_worked, T.task_type, T.task_minutes, T.task_id
         FROM timesheets AS TS
         INNER JOIN locations AS L ON TS.timesheet_id = L.timesheet_id
         INNER JOIN tasks AS T ON L.date_worked = T.date_worked AND TS.week_ending = L.week_ending AND L.location_id = T.location_id
         WHERE TS.employee_id = ?
         AND TS.week_ending = ?
         ORDER BY L.location;`;
    pool.query(timesheetJoinSql, timesheetData, (err, timesheetRows) => {
      ...
      let groupByDateWorked = _.groupBy(timesheetRows, "date_worked");
      let dates = _.keys(groupByDateWorked);
      let timesheet = _.sortBy(timesheetRows, ["date_worked", "location"]);
